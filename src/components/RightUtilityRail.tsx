      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <a
            ...
            className="text-[11px] font-semibold text-neutral-800 hover:text-neutral-950 flex items-center gap-0.5"
          >
        ...
        <div className="p-3.5 rounded-lg border border-neutral-300 bg-white/70 backdrop-blur-md hover:bg-white/95 hover:border-neutral-400 transition-all flex items-start gap-3 shadow-2xs">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-mono font-bold px-1.5 py-0.2 rounded bg-neutral-100 text-neutral-800 border border-neutral-200">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-[10px] text-emerald-800 font-semibold bg-emerald-50 border border-emerald-200 rounded-md px-2 py-0.5 w-fit">
          ...
          {discussionCards.map((card) => (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-1 border-t border-neutral-100 text-[11px]">
                <span className="text-neutral-700 hover:text-neutral-950 font-semibold flex items-center gap-1 cursor-pointer">
                  <MessageSquare className="w-3 h-3" />
                  {card.comments.length > 0 ? `${card.comments.length} replies` : 'Reply →'}
                </span>
                {card.isPinned && (
                  <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded bg-neutral-100 text-neutral-800 border border-neutral-200 font-mono">
                    PINNED
                  </span>
                )}
              </div>
        ...
        <div className="space-y-1.5">
          {topContributors.slice(0, 4).map((author, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-0.5 text-[11px] font-bold font-mono text-neutral-800 shrink-0">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-[11px] font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5">
            <FileText className="w-3.5 h-3.5 text-neutral-600" /> Pinned Resources
          </span>
          ...
          {pinnedResources.map((res) => {
            ...
            return (
              <button
                ...
                className="w-full text-left p-2.5 rounded-lg bg-white/70 backdrop-blur-md border border-neutral-300 hover:border-neutral-400 hover:bg-white hover:shadow-2xs transition-all flex items-center justify-between group cursor-pointer"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="p-1 rounded-lg bg-neutral-100 group-hover:bg-neutral-200/70 transition-colors">
                    {icon}
                  </span>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-neutral-800 group-hover:text-neutral-950 transition-colors truncate">
                      {res.title}
                    </p>
                ...
                <Download className="w-3.5 h-3.5 text-neutral-400 group-hover:text-neutral-900 shrink-0 transition-colors" />
              </button>
            )
          })}
        </div>
